...
def save_latents_to_disk(npz_path, latents_tensor, original_size, crop_ltrb, flipped_latents_tensor):
    kwargs = {}
...
def cache_batch_latents(
    vae: AutoencoderKL, cache_to_disk: bool, image_infos: List[train_util.ImageInfo], flip_aug: bool, random_crop: bool
) -> None:
    r"""
    ...
    def process_batch(is_last):
        for bucket in bucket_manager.buckets:
            if (is_last and len(bucket) > 0) or len(bucket) >= args.batch_size:
                cache_batch_latents(vae, True, bucket, args.flip_aug, False)
                bucket.clear()
